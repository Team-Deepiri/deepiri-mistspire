...
| Headset walkthrough | `./setup.sh` (interactive; `--non-interactive` skips) | `.\setup.ps1` (`-NonInteractive` skips) | Skip; use native OS for VR |
| System deps | `scripts/setup-linux-deps.sh` (apt / pacman / SteamOS / dnf) | `scripts/setup-windows-deps.ps1` (checks) | Linux deps script (no VR) |
...
`setup.sh` / `setup.ps1` ask about VR headsets before deps and point at [headsets/README.md](headsets/README.md). Use `--non-interactive` / `-NonInteractive` for CI or scripted runs.

Linux deps (`setup-linux-deps.sh`) auto-install on **Debian/Ubuntu** (`apt`), **Arch** and Arch-based (`pacman`), **SteamOS** / Steam Deck (`pacman` + `steamos-readonly`; never `-Syu`), and **Fedora** (`dnf`). Other distros: install cmake, Vulkan, OpenXR loader, and git-lfs manually.

On SteamOS, pacman packages are wiped by the next `steamos-update` — re-run the deps script after OS updates. Prefer a desktop Linux or Windows PC for Unreal Editor; Deck/Desktop Mode is fine for git and `xr-sandbox` / OpenXR checks.
